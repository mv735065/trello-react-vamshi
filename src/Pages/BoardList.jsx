...
import useBoardNavBarStyles from "../Styles/UseBoardNavBarStyles";
...
const BoardList = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        dispatch({
          type: "Error",
          error: "Unable to fetch lists of board " + err.message,
        });
      } finally {
  ...
  async function handleAddNewList(event) {
    ...
    try {
      ...
      dispatch({
        type: "Error",
        error: "Error creating board: " + err.message,
      });
    }
  }
  ...
  return listsOfBoard.loading ? (
    ...
    <ErrorPage errorMessage={listsOfBoard.error} />
  ) : (
    <Box
      sx={{
        ...styles.mainBox,
